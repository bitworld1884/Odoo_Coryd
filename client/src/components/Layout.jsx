      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {user?.isAdmin && (
              <NavLink to="/app/admin" className="hidden rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-semibold text-white sm:inline">Admin</NavLink>
            )}
          ...
          <nav className="space-y-1">
            ...
            {user?.isAdmin && (
              <NavLink to="/app/admin" className={({ isActive }) =>
                `block rounded-lg px-3 py-2 text-sm font-medium sm:hidden ${isActive ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
                Admin
              </NavLink>
            )}
          </nav>
